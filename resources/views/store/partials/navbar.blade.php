<header x-data="{ mobileOpen: false, scrolled: false, megaMenu: null }" @scroll.window="scrolled = (window.pageYOffset > 20)"
            ...
            <div class="flex items-center gap-8">
                ...
                <a href="/" class="relative z-10 block group">
                    <img src="{{ asset('storage/site_images/logo1dotcom.png') }}" alt="ExamBabu"
                        class="object-cover w-16 h-16 transition-transform duration-300 border-2 border-white rounded-full shadow-lg group-hover:rotate-12">
                </a>
                <nav class="hidden gap-1 md:flex">
                    <div class="relative" @mouseenter="megaMenu = 'exams'" @mouseleave="megaMenu = null">
                        <button
                            class="flex items-center gap-1 px-4 py-2 text-sm font-bold transition-all rounded-full text-slate-700 hover:bg-slate-100"
                            :style="megaMenu === 'exams' ? 'color: var(--brand-blue);' : ''">
                            Exams
                            <svg class="w-4 h-4 transition-transform duration-300"
                                :class="megaMenu === 'exams' ? 'rotate-180' : 'text-slate-400'"
                                :style="megaMenu === 'exams' ? 'color: var(--brand-blue);' : ''" fill="none"
                                ...
                                @if (isset($examCategories))
                                    @foreach ($examCategories as $catName => $data)
                                        <button @mouseenter="activeCat = '{{ $catName }}'"
                                            ...
                                            :class="activeCat === '{{ $catName }}' ? 'bg-white shadow-sm' :
                                                'text-slate-600 border-transparent hover:bg-slate-100'"
                                            :style="activeCat === '{{ $catName }}' ?
                                                'color: var(--brand-blue); border-color: var(--brand-blue);' : ''">
                                            <span class="text-base">{{ $data['icon'] }}</span>
                                            {{ $catName }}
                                            <svg x-show="activeCat === '{{ $catName }}'" class="w-4 h-4 ml-auto"
                                                style="color: var(--brand-blue);" fill="none" viewBox="0 0 24 24"
                                                stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                    d="M9 5l7 7-7 7" />
                                            </svg>
                                    ...
                                    @foreach ($examCategories as $catName => $data)
                                        <div x-show="activeCat === '{{ $catName }}'" class="flex flex-col h-full">
                                            <div
                                                ...
                                                <a href="#"
                                                    class="flex items-center gap-1 text-xs font-bold hover:underline"
                                                            ...
                                                            <div class="grid grid-cols-2 gap-2">
                                                                @foreach ($exams as $exam)
                                                                    <a href="#"
                                                                        ...
                                                                        <div class="w-1.5 h-1.5 rounded-full bg-slate-300 transition-colors"
                                                                            style="group-hover:background-color: var(--brand-blue);">
                                                                        </div>
                                                                        <span
                                                                            class="text-sm font-medium truncate text-slate-700"
                                                                            style="group-hover:color: var(--brand-blue);"
                                                    ...
                                                    @foreach ($data['exams'] as $exam)
                                                        <a href="#"
                                                            ...
                                                            <span class="text-sm font-semibold text-slate-700"
                                                                style="group-hover:color: var(--brand-blue);">{{ $exam }}</span>
                                                        </a>
                                                    @endforeach
                                                </div>
                                            @endif
                    ...
                    <a href="#"
                        class="px-4 py-2 text-sm font-bold transition-colors rounded-full text-slate-700 hover:bg-slate-100"
                        style="hover:color: var(--brand-blue);">Test Series</a>
                    <a href="#"
                        class="px-4 py-2 text-sm font-bold transition-colors rounded-full text-slate-700 hover:bg-slate-100"
                        style="hover:color: var(--brand-blue);">Super Coaching</a>
                    <a href="#"
                        class="px-4 py-2 text-sm font-bold transition-colors rounded-full text-slate-700 hover:bg-slate-100"
                        style="hover:color: var(--brand-blue);">Pass Pro</a>
                </nav>
            </div>
    ...
    <div x-show="mobileOpen" x-transition class="fixed inset-0 z-50 overflow-y-auto bg-white md:hidden">
        ...
        <div class="p-4 space-y-4">
            ...
            <a href="{{ route('register') }}" class="block w-full py-3 font-bold text-center text-white rounded-xl"
                style="background-color: var(--brand-blue);">Sign up Free</a>
        </div>
    </div>
</header>
