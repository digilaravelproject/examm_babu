    ...
    <div class="flex h-screen overflow-hidden" x-data="{ sidebarOpen: false, fileManagerOpen: false }">
        ...
        <aside class="fixed inset-y-0 left-0 z-30 flex flex-col w-64 h-full transition-all duration-300 transform border-r bg-slate-900 border-slate-800 md:static md:translate-x-0"
            ...
            <nav class="flex-1 px-3 py-4 space-y-1 overflow-y-auto custom-scroll">
                ...
                <button @click="fileManagerOpen = true" class="flex items-center w-full px-4 py-2.5 text-sm font-medium rounded-lg text-slate-400 hover:bg-slate-800 hover:text-white transition-all">
                    <svg class="w-5 h-5 mr-3 text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z"></path></svg>
                    File Manager
                </button>

                <div class="pt-4 pb-1 pl-4 uppercase text-[10px] font-bold tracking-widest text-slate-600">Engagement</div>
                ...
                <div x-data="{ open: {{ request()->routeIs('admin.quizzes.*', 'admin.exam.*', 'admin.quiz-types.*', 'admin.exam-types.*') ? 'true' : 'false' }} }">
                    ...
                    <div x-show="open" x-cloak x-collapse class="mt-1 space-y-1 bg-slate-800/30 rounded-lg mx-2">
                        ...
                        <a href="{{ Route::has('admin.exam-types.index') ? route('admin.exam-types.index') : '#' }}" class="block px-8 py-2 text-sm {{ request()->routeIs('admin.exam-types.*') ? 'sub-link-active' : 'text-slate-400 hover:text-white' }}">Exam Types</a>
                    </div>
                </div>

                <div x-data="{ open: {{ request()->routeIs('admin.practice-sets.*', 'admin.lessons.*', 'admin.videos.*') ? 'true' : 'false' }} }">
                    <button @click="open = !open" class="flex items-center justify-between w-full px-4 py-2.5 text-sm font-medium rounded-lg text-slate-400 hover:bg-slate-800 hover:text-white">
                        <div class="flex items-center">
                ...
                <div x-data="{ open: {{ request()->routeIs('admin.questions.*', 'admin.comprehensions.*', 'admin.question-types.*') ? 'true' : 'false' }} }">
                    ...
                    <div x-show="open" x-cloak x-collapse class="mt-1 space-y-1 bg-slate-800/30 rounded-lg mx-2">
                        ...
                        <a href="{{ Route::has('admin.question-types.index') ? route('admin.question-types.index') : '#' }}" class="block px-8 py-2 text-sm {{ request()->routeIs('admin.question-types.*') ? 'sub-link-active' : 'text-slate-400 hover:text-white' }}">Question Types</a>
                    </div>
                </div>
