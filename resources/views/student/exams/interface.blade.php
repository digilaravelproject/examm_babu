    ...
    <div x-show="showInstructions" class="fixed inset-0 z-[100] bg-white overflow-y-auto">
        ...
        <div class="max-w-5xl p-8 mx-auto">
            <div class="flex items-center gap-4 mb-6">
                <img src="https://ui-avatars.com/api/?name={{ urlencode($user->name) }}&background=0D8ABC&color=fff" class="w-16 h-16 border-4 border-gray-200 rounded-full">
                <div>
                ...
                <div class="max-w-4xl mx-auto">
                    <div class="flex items-center justify-between pb-4 mb-6 bg-white border-b border-gray-200">
                        ...
                        <div class="flex gap-2 text-xs font-bold text-gray-600">
                            <span class="px-2 py-1 rounded bg-green-50">+<span x-text="currentQuestion?.marks"></span></span>
                            <span class="px-2 py-1 rounded bg-red-50">-<span x-text="currentQuestion?.negative"></span></span>
                        </div>
                    </div>
                    ...
                    <div class="space-y-4">
                        <template x-for="(opt, idx) in getOptions()" :key="idx">
                            <div @click="selectOption(idx)"
                                 ...
                                 :class="currentQuestion.selected_option === idx ? 'border-[#3498db] bg-blue-50' : 'border-gray-200 hover:bg-gray-50'">
                                <div class="flex items-center justify-center w-6 h-6 mr-4 border-2 rounded-full shrink-0"
                                     :class="currentQuestion.selected_option === idx ? 'border-blue-600 bg-blue-600' : 'border-gray-400'">
                                    <div class="w-2.5 h-2.5 bg-white rounded-full" x-show="currentQuestion.selected_option === idx"></div>
                                </div>
    ...
    <script>
        function examEngine(sectionsMeta, duration, sessionCode, fetchUrlTemplate, saveUrl, suspendUrl, finishUrl) {
            return {
                ...
                // --- NAVIGATION ---
                switchSection(idx) { this.loadSectionData(idx); },
                ...
                selectOption(idx) { this.currentQuestion.selected_option = idx; },
                ...
                async checkTranslation() {
                    ...
                    // Mock Translation Logic (Replace with API)
                    try {
                ...
                getPaletteClass(q, idx) {
                    let cls = "";
                    if (q.status === 'not_visited') cls = 'bg-white border';
