...
@php
    ...
    // Pre-generate URLs
    $urlCreate = route($routePrefix . 'exams.create', $routeParams);
...
{{-- Pass Configuration to Alpine --}}
<div x-data="examList({ baseUrl: '{{ $urlIndex }}' })" x-init="init()" class="space-y-6">
    ...
    <div class="flex flex-col justify-between gap-4 md:flex-row md:items-center">
        ...
        {{-- Create Button using Brand Blue (Dynamic URL) --}}
        <a href="{{ $urlCreate }}"
    ...
    {{-- Table Container --}}
    {{-- Pass dynamic variables to partial as well --}}
    <div x-show="!loading" id="exam-table-container" class="transition-all duration-300">
        @include('admin.exams.partials.table', ['routePrefix' => $routePrefix, 'routeParams' => $routeParams])
    </div>
</div>
@endsection
...
@push('scripts')
<script>
    // Accept config to use dynamic URL
    function examList(config) {
        return {
            search: '',
            type: '',
            status: '',
            ...
            fetchExams(url = null) {
                // Use passed URL (pagination) or default base URL
