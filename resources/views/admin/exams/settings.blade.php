...
@php
    ...
    // Merge Exam ID for update route
    $updateParams = array_merge($routeParams, ['exam' => $exam->id]);
    $urlUpdate = route($routePrefix . 'exams.settings.update', $updateParams);
@endphp

@section('content')
<div class="max-w-5xl py-8 mx-auto">
    {{-- Steps Navigation --}}
    @include('admin.exams.partials._steps', ['activeStep' => 'settings', 'routePrefix' => $routePrefix, 'routeParams' => $routeParams])
    ...
    <div class="mt-8">
        ...
        {{-- Dynamic Form Action --}}
        <form action="{{ $urlUpdate }}" method="POST" x-data="examSettings()">
            @csrf

            <div class="grid grid-cols-1 gap-8 lg:grid-cols-2">

                {{-- CARD 1: SCORING & TIMING --}}
                <div class="p-8 bg-white border border-t-4 border-gray-200 shadow-sm rounded-2xl h-fit" style="border-t-color: var(--brand-blue);">
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="space-y-2" x-data="{ open: false, selected: '{{ $settings['duration_mode'] == 'auto' ? 'Auto (Sum of Questions)' : 'Manual (Fixed Duration)' }}', value: '{{ $settings['duration_mode'] }}' }">
                            ...
                            <div class="relative">
                                ...
                                <div x-show="open" x-transition class="absolute z-50 w-full mt-2 bg-white border border-gray-100 shadow-xl rounded-xl">
                        ...
                        <div class="pt-2 space-y-4">
                            <div class="flex items-center justify-between p-4 border border-gray-100 bg-gray-50/50 rounded-xl">
                                <span class="text-sm font-bold text-gray-700">Sectional Cutoff</span>
                                <button type="button" @click="sectionCutoff = !sectionCutoff" :class="sectionCutoff ? 'bg-[var(--brand-blue)]' : 'bg-gray-300'" class="relative inline-flex h-6 transition-colors rounded-full w-11">
                            ...
                            <div class="flex items-center justify-between p-4 border border-gray-100 bg-gray-50/50 rounded-xl">
                                ...
                                <button type="button" @click="negativeMarking = !negativeMarking" :class="negativeMarking ? 'bg-[var(--brand-pink)]' : 'bg-gray-300'" class="relative inline-flex h-6 transition-colors rounded-full w-11">
                ...
                <div class="p-8 bg-white border border-t-4 border-gray-200 shadow-sm rounded-2xl h-fit" style="border-t-color: var(--brand-sky);">
                    <h3 class="mb-6 text-[11px] font-black text-gray-400 uppercase tracking-[0.2em]">Access & Restrictions</h3>

                    <div class="space-y-4">
                        {{-- Restrict Attempts Card --}}
                        <div class="p-5 transition-all border border-gray-100 bg-gray-50/50 rounded-2xl" :class="restrictAttempts ? 'ring-2 ring-[var(--brand-blue)]/20 bg-white' : ''">
                            <div class="flex items-center justify-between">
                                ...
                                <button type="button" @click="restrictAttempts = !restrictAttempts" :class="restrictAttempts ? 'bg-[var(--brand-blue)]' : 'bg-gray-300'" class="relative inline-flex h-6 transition-colors rounded-full w-11">
                            ...
                            <div x-show="restrictAttempts" x-transition class="pt-4 mt-4 border-t border-gray-100">
                        ...
                        <div x-data="{ active: {{ $settings[$set['name']] ? 'true' : 'false' }} }"
                             class="flex items-center justify-between p-4 border border-gray-100 bg-gray-50/50 rounded-xl">
                            ...
                            <button type="button" @click="active = !active" :class="active ? 'bg-[var(--brand-blue)]' : 'bg-gray-300'" class="relative inline-flex h-6 transition-colors rounded-full w-11">
                ...
                <div class="p-8 bg-white border border-t-4 border-gray-200 shadow-sm rounded-2xl md:col-span-2" style="border-t-color: var(--brand-green);">
                    ...
                    <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
                        @foreach([
                            ...
                            <div x-data="{ active: {{ $settings[$report['name']] ? 'true' : 'false' }} }"
                                 class="flex items-center justify-between p-5 transition-all border border-gray-100 bg-gray-50/30 rounded-2xl hover:bg-white hover:shadow-sm">
                                <span class="text-sm font-bold text-gray-700">{{ $report['label'] }}</span>
                                <button type="button" @click="active = !active" :class="active ? 'bg-[var(--brand-green)]' : 'bg-gray-300'" class="relative inline-flex h-6 transition-colors rounded-full w-11">
            ...
            {{-- FOOTER BUTTON --}}
            <div class="flex items-center justify-end pb-10 mt-10">
                <button type="submit" class="flex items-center gap-3 px-12 py-4 text-sm font-black text-white transition-all shadow-xl rounded-2xl hover:shadow-2xl hover:-translate-y-1 active:scale-95" style="background-color: var(--brand-blue);">
