...
<div class="px-4 py-8 mx-auto max-w-7xl sm:px-6 lg:px-8"
     x-data="scheduleManager({ urlPrefix: '{{ $urlPrefix }}' })"
     @keydown.escape.window="closeModal()">

    {{-- Steps Navigation --}}
    @if(view()->exists('admin.exams.partials._steps'))
        @include('admin.exams.partials._steps', ['activeStep' => 'schedules', 'routePrefix' => $routePrefix, 'routeParams' => $routeParams])
    @endif
    ...
    <div class="mt-8 space-y-6">
        ...
        <div class="overflow-hidden bg-white border border-gray-100 shadow-xl rounded-2xl">
                    ...
                    <tr class="transition-colors hover:bg-blue-50/30 group">
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end gap-1">
                                {{-- Analytics (Dynamic) --}}
                                <a href="{{ route($routePrefix . 'exams.schedules.analytics', array_merge($routeParams, ['exam' => $exam->id, 'schedule' => $schedule->id])) }}"
                                ...
                                {{-- Delete (Dynamic) --}}
                                <form action="{{ route($routePrefix . 'exams.schedules.destroy', array_merge($routeParams, ['exam' => $exam->id, 'schedule' => $schedule->id])) }}" method="POST" onsubmit="return confirm('Delete this schedule?')">
                                    @csrf @method('DELETE')
            ...
            <div class="p-6 border-t border-gray-100 bg-gray-50/50">
                {{ $schedules->appends($routeParams)->links() }}
            </div>
        </div>
    </div>
...
function scheduleManager(config) {
    return {
        ...
        urlPrefix: config.urlPrefix, // passed from blade

        openAddModal() {
            this.isEdit = false;
            // FIX: Dynamic URL
            this.formAction = `/${this.urlPrefix}/exams/{{ $exam->id }}/schedules`;
            this.resetForm();
        ...
        async editSchedule(examId, scheduleId) {
            try {
                // FIX: Dynamic URL
                const res = await fetch(`/${this.urlPrefix}/exams/${examId}/schedules/${scheduleId}/edit`);
                const data = await res.json();

                this.isEdit = true;
                // FIX: Dynamic URL
                this.formAction = `/${this.urlPrefix}/exams/${examId}/schedules/${scheduleId}`;
