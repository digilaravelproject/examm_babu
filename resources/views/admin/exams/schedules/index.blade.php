...
@section('content')
<div class="max-w-7xl py-8 mx-auto px-4 sm:px-6 lg:px-8"
    ...
    <div class="mt-8 space-y-6">
        {{-- Header Section --}}
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
            <div>
                <h1 class="text-2xl font-black text-gray-900 tracking-tight">Exam Schedules</h1>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-gray-50/80 border-b border-gray-100">
                        <th class="px-6 py-4 text-xs font-black text-gray-400 uppercase tracking-widest">Access Code</th>
                        <th class="px-6 py-4 text-xs font-black text-gray-400 uppercase tracking-widest text-center">Type</th>
                        <th class="px-6 py-4 text-xs font-black text-gray-400 uppercase tracking-widest">Start Schedule</th>
                        <th class="px-6 py-4 text-xs font-black text-gray-400 uppercase tracking-widest">End Schedule</th>
                    ...
                    <tr class="hover:bg-blue-50/30 transition-colors group">
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-3">
                                ...
                                <span class="font-mono font-bold text-gray-700 tracking-tighter">{{ $schedule->code ?? 'SCH-'.$schedule->id }}</span>
                            </div>
                        </td>
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end gap-1">
                                ...
                                <a href="{{ Route::has('admin.exams.schedules.analytics') ? route('admin.exams.schedules.analytics', [$exam->id, $schedule->id]) : '#' }}"
                                   class="p-2 text-gray-400 hover:text-amber-500 hover:bg-amber-50 rounded-lg transition-all" title="Analytics">
                                ...
                                <form action="{{ route('admin.exams.schedules.destroy', [$exam->id, $schedule->id]) }}" method="POST" onsubmit="return confirm('Delete this schedule?')">
                                    @csrf @method('DELETE')
                                    <button class="p-2 text-gray-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-all">
            ...
            <div class="inline-block w-full max-w-2xl overflow-hidden text-left align-bottom transition-all transform bg-white shadow-2xl rounded-3xl sm:my-8 sm:align-middle">
                ...
                <form :action="formAction" method="POST" id="scheduleForm">
                    ...
                    <div class="px-8 py-6 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
                        ...
                        <button type="button" @click="closeModal()" class="p-2 text-gray-400 hover:text-gray-600 bg-white rounded-xl border border-gray-100 shadow-sm">
                    ...
                    <div class="p-8 space-y-6">
                        ...
                        <div class="bg-gray-50 p-4 rounded-2xl">
                            <label class="block mb-3 text-xs font-black text-gray-400 uppercase tracking-widest">Schedule Strategy</label>
                            <div class="grid grid-cols-2 gap-4">
                                <label class="relative flex flex-col p-4 bg-white border-2 rounded-xl cursor-pointer transition-all"
                        ...
                        <div class="relative" x-data="{ open: false, search: '' }">
                            <label class="block mb-2 text-xs font-black text-gray-400 uppercase tracking-widest">Assign to Groups</label>
                            <div class="min-h-[50px] w-full p-2 bg-white border-2 border-gray-100 rounded-xl flex flex-wrap gap-2 cursor-pointer focus-within:border-[#0777be]" @click="open = !open">
                                ...
                                <span x-show="selectedGroups.length === 0" class="text-gray-400 text-sm py-1 px-2">Select user groups...</span>
                            ...
                            <div x-show="open" @click.away="open = false" class="absolute z-[60] w-full mt-2 bg-white border border-gray-100 shadow-2xl rounded-2xl max-h-60 overflow-y-auto p-2">
                                <input type="text" x-model="search" placeholder="Search groups..." class="w-full mb-2 p-2 text-sm border-gray-100 rounded-lg bg-gray-50 focus:ring-0">
                                <div class="space-y-1">
                                    @foreach($userGroups as $group)
                                    <div x-show="'{{ strtolower($group->name) }}'.includes(search.toLowerCase())"
                                         @click="toggleGroup({{ $group->id }})"
                                         class="px-4 py-2 text-sm font-medium rounded-lg cursor-pointer flex items-center justify-between"
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="space-y-2">
                                <label class="block text-xs font-black text-gray-400 uppercase tracking-widest">Start Date</label>
                            ...
                            <div class="space-y-2">
                                <label class="block text-xs font-black text-gray-400 uppercase tracking-widest">Start Time</label>
                            ...
                            <template x-if="scheduleType === 'flexible'">
                                <div class="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6 p-4 bg-orange-50/30 border border-orange-100 rounded-2xl animate-fadeIn">
                                    <div class="space-y-2">
                                        <label class="block text-xs font-black text-orange-400 uppercase tracking-widest">End Date</label>
                                    ...
                                    <div class="space-y-2">
                                        <label class="block text-xs font-black text-orange-400 uppercase tracking-widest">End Time</label>
                        ...
                        {{-- Grace Period --}}
                        <div class="p-4 bg-blue-50/30 border border-blue-100 rounded-2xl flex items-center gap-6">
                            ...
                            <div class="flex-grow">
                                <label class="block text-xs font-black text-gray-400 uppercase tracking-widest">Grace Period (Minutes)</label>
                    ...
                    {{-- Modal Footer --}}
                    <div class="px-8 py-6 bg-gray-50 border-t border-gray-100 rounded-b-3xl flex items-center justify-between">
