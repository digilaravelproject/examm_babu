...
@if ($errors->any())
    <div class="p-4 mb-6 border-l-4 rounded-lg bg-red-50 border-red-500/50">
        <div class="flex items-center mb-2">
            ...
            <span class="text-sm font-bold tracking-tight text-red-800 uppercase">Please Fix Errors:</span>
        </div>
...
<div x-data="examForm()" class="grid grid-cols-1 gap-x-8 gap-y-6 md:grid-cols-2">
    ...
    <div class="space-y-2" x-data="{ open: false }">
        ...
        <div class="relative">
            ...
            <div x-show="open" x-transition
                class="absolute z-50 w-full mt-2 overflow-y-auto bg-white border border-gray-100 shadow-xl rounded-xl max-h-60 no-scrollbar"
                style="display: none;">
    ...
    <div class="space-y-2" x-data="{ open: false }">
        ...
        <div class="relative">
            ...
            <div x-show="open" x-transition
                ...
                <div x-show="filteredSubCats.length === 0" class="px-4 py-3 text-sm text-center text-gray-400">
    ...
    <div class="space-y-2" x-data="{ open: false }">
        ...
        <div class="relative">
            ...
            <div x-show="open" x-transition
                ...
                <div x-show="filteredMicros.length === 0" class="px-4 py-3 text-sm text-center text-gray-400">
    ...
    @php
        $selectedTypeName = $exam->exam_type_id ? optional($examTypes->firstWhere('id', $exam->exam_type_id))->name : 'Select Exam Type';
    @endphp
    <div class="space-y-2 md:col-span-2" x-data="{ open: false, selected: '{{ $selectedTypeName }}', value: '{{ old('exam_type_id', $exam->exam_type_id) }}' }">
    ...
    <div class="grid grid-cols-1 gap-4 md:grid-cols-2 md:col-span-2">
        ...
        {{-- Topic Toggle --}}
        <div x-data="{ enableTopic: {{ old('topic_id', $exam->topic_id ?? false) ? 'true' : 'false' }}, topicId: '{{ old('topic_id', $exam->topic_id) }}' }" class="grid grid-cols-12 col-span-2 gap-4 mt-6">
            {{-- LEFT: Topic Toggle --}}
            <div class="col-span-12">
                <div
                    class="flex items-center justify-between h-full p-4 transition-all border border-gray-100 bg-gray-50/80 rounded-2xl hover:bg-gray-50">
                    <div>
                        ...
                        {{-- FIX: Added hidden input for when topic is disabled --}}
                        <input type="hidden" name="topic_id" :value="enableTopic ? topicId : ''">
                    </div>
            ...
            <div class="col-span-12" x-show="enableTopic" x-transition>
                <div class="h-full p-4 border border-gray-100 bg-gray-50/80 rounded-2xl">
                    <label class="text-[11px] font-black text-gray-500 uppercase tracking-widest block mb-2">Select
                        Topic</label>
                    <select x-model="topicId" x-bind:disabled="!enableTopic"
    ...
    <div class="flex items-center justify-end pt-4 mt-2 md:col-span-2">
        <button type="submit"
            class="flex items-center gap-3 px-10 py-4 text-sm font-bold text-white transition-all shadow-lg rounded-xl hover:shadow-xl hover:-translate-y-1 active:scale-95"
...
<script>
    function examForm() {
        return {
            ...
            categoryName: '{{ $exam->category_id ? optional($categories->firstWhere('id', $exam->category_id))->name ?? 'Selected' : '' }}',

            subCategoryId: '{{ old('sub_category_id', $exam->sub_category_id) }}',
            subCategoryName: '{{ $exam->sub_category_id ? optional($subCategories->firstWhere('id', $exam->sub_category_id))->name ?? 'Selected' : '' }}',

            microCategoryId: '{{ old('micro_category_id', $exam->micro_category_id) }}',
            microCategoryName: '{{ $exam->micro_category_id ? optional($microCategories->firstWhere('id', $exam->micro_category_id))->name ?? 'Selected' : '' }}',
