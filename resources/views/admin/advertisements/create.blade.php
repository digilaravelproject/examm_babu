...
@section('header', 'Add Advertisement')
...
<div class="max-w-4xl mx-auto">
    ...
    <div class="bg-white border border-gray-200 shadow-sm rounded-xl">
        <div class="px-6 py-4 border-b border-gray-100">
            <h2 class="text-lg font-bold text-gray-900">Create New Advertisement</h2>
        </div>

        <form action="{{ route('admin.advertisements.store') }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-2">
                    ...
                    <select name="location" id="location" required
                        ...
                        <option value="report_banner" {{ old('location') == 'report_banner' ? 'selected' : '' }}>Report Banner (Top/Bottom)</option>
                        <option value="sidebar" {{ old('location') == 'sidebar' ? 'selected' : '' }}>Sidebar Ad</option>
                    </select>
                </div>

                {{-- Image Upload --}}
                <div class="space-y-2 md:col-span-2">
                    <label for="image" class="block text-sm font-semibold text-gray-700">Banner Image <span class="text-red-500">*</span></label>
            ...
            <div class="flex justify-end gap-3 pt-6 border-t border-gray-100">
                ...
                <button type="submit" class="px-8 py-2 text-sm font-bold text-white bg-indigo-600 rounded-lg hover:bg-indigo-700 shadow-md">Save Advertisement</button>
            </div>
        </form>
    </div>
</div>
