@php
    // FIX: Fallback logic if variables are not passed from parent
    if (!isset($routePrefix)) {
        $isAdmin = request()->routeIs('admin.*');
        $routePrefix = $isAdmin ? 'admin.' : 'panel.';
    }

    if (!isset($routeParams)) {
        $routeParams = [];
        if ($routePrefix === 'panel.') {
            $routeParams = ['role' => request()->route('role') ?? 'instructor'];
        }
    }
@endphp

<div class="overflow-hidden bg-white border border-gray-200 shadow-sm rounded-xl">
    <div class="overflow-x-auto">
        <table class="w-full text-left border-collapse">
            <thead class="border-b border-gray-200 bg-gray-50">
                <tr>
                    <th class="w-20 px-3 py-3 text-xs font-bold tracking-wider text-gray-500 uppercase">Code</th>
                    <th class="px-4 py-3 text-xs font-bold tracking-wider text-gray-500 uppercase min-w-[200px]">Title</th>
                    <th class="px-4 py-3 text-xs font-bold tracking-wider text-gray-500 uppercase">Preview</th>

                    {{-- NEW: Creator Column (Admin Only) --}}
                    @if(auth()->user()->hasRole('admin'))
                        <th class="px-4 py-3 text-xs font-bold tracking-wider text-gray-500 uppercase">Created By</th>
                    @endif

                    <th class="px-4 py-3 text-xs font-bold tracking-wider text-center text-gray-500 uppercase">Status</th>
                    <th class="px-4 py-3 text-xs font-bold tracking-wider text-center text-gray-500 uppercase">Questions</th>
                    <th class="px-4 py-3 text-xs font-bold tracking-wider text-center text-gray-500 uppercase">Created</th>
                    <th class="px-4 py-3 text-xs font-bold tracking-wider text-right text-gray-500 uppercase">Actions</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-gray-100">
                @forelse($passages as $passage)
                    <tr class="transition-colors hover:bg-gray-50/80 group">
                        {{-- Code --}}
                        <td class="px-3 py-4">
                            <span class="px-1.5 py-0.5 font-mono text-[11px] font-medium bg-blue-50 text-[#0777be] rounded whitespace-nowrap">
                                {{ $passage->code }}
                            </span>
                        </td>

                        {{-- Title --}}
                        <td class="px-4 py-4">
                            <div class="text-sm font-medium text-gray-900 line-clamp-1" title="{{ $passage->title }}">
                                {{ $passage->title }}
                            </div>
                        </td>

                        {{-- Body Preview --}}
                        <td class="px-4 py-4">
                            <div class="text-xs italic text-gray-500 line-clamp-1">
                                {{ Str::limit(strip_tags($passage->body), 60) }}
                            </div>
                        </td>

                        {{-- NEW: Creator Info (Admin Only) --}}
                       @if(auth()->user()->hasRole('admin'))
                            <td class="px-4 py-4">
                                <div class="flex flex-col">
                                    <span class="text-xs font-bold text-gray-700">
                                        {{-- Check if creator exists --}}
                                        {{ $passage->creator ? $passage->creator->fullname : 'Unknown' }}
                                    </span>
                                    <span class="text-[10px] text-gray-500">
                                        {{-- Check if creator exists AND has roles --}}
                                        @if($passage->creator && $passage->creator->roles->isNotEmpty())
                                            {{ $passage->creator->roles->first()->name }}
                                        @else
                                            --
                                        @endif
                                    </span>
                                </div>
                            </td>
                        @endif

                        {{-- Status --}}
                        <td class="px-4 py-4 text-center">
                            @if ($passage->is_active)
                                <span class="inline-flex items-center px-2 py-1 text-xs font-medium rounded-full bg-[#94c940]/10 text-[#94c940] whitespace-nowrap">
                                    <span class="w-1.5 h-1.5 mr-1.5 rounded-full bg-[#94c940]"></span> Active
                                </span>
                            @else
                                <span class="inline-flex items-center px-2 py-1 text-xs font-medium text-orange-600 rounded-full bg-orange-50 whitespace-nowrap">
                                    <span class="w-1.5 h-1.5 mr-1.5 rounded-full bg-orange-500"></span> Inactive
                                </span>
                            @endif
                        </td>

                        {{-- Questions Count --}}
                        <td class="px-4 py-4 text-center">
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                                {{ $passage->questions_count }}
                            </span>
                        </td>

                        {{-- Date --}}
                        <td class="px-4 py-4 text-center">
                            <span class="text-xs text-gray-500">{{ $passage->created_at->format('d M, Y') }}</span>
                        </td>

                        {{-- Actions --}}
                        <td class="px-4 py-4 text-right">
                            <div class="flex items-center justify-end gap-2">
                                {{-- Usage Stats (Info Blue) --}}
                                <a href="{{ route($routePrefix . 'comprehensions.usage', array_merge($routeParams, ['comprehension' => $passage->id])) }}"
                                   class="flex items-center justify-center w-8 h-8 transition-all bg-white border border-gray-200 rounded-lg shadow-sm hover:bg-indigo-50 hover:border-indigo-200 hover:text-indigo-600 group/btn"
                                   title="View Usage">
                                    <svg class="w-4 h-4 text-gray-500 group-hover/btn:text-indigo-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
                                    </svg>
                                </a>

                                {{-- Edit (Pink Hover) --}}
                                {{-- FIX: Dynamic Route --}}
                                <a href="{{ route($routePrefix . 'comprehensions.edit', array_merge($routeParams, ['comprehension' => $passage->id])) }}"
                                    class="flex items-center justify-center w-8 h-8 transition-all bg-white border border-gray-200 rounded-lg shadow-sm hover-edit-btn group/btn"
                                    title="Edit">
                                    <svg class="w-4 h-4 text-gray-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                    </svg>
                                </a>

                                {{-- Delete (Red Hover) --}}
                                {{-- FIX: Dynamic Route --}}
                                <form action="{{ route($routePrefix . 'comprehensions.destroy', array_merge($routeParams, ['comprehension' => $passage->id])) }}" method="POST"
                                      onsubmit="return confirm('Irreversible action! Are you sure?');">
                                    @csrf @method('DELETE')
                                    <button type="submit"
                                        class="flex items-center justify-center w-8 h-8 transition-all bg-white border border-gray-200 rounded-lg shadow-sm hover:bg-red-500 hover:border-red-500 hover:text-white group/btn"
                                        title="Delete">
                                        <svg class="w-4 h-4 text-gray-500 group-hover/btn:text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                                        </svg>
                                    </button>
                                </form>
                            </div>
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="{{ auth()->user()->hasRole('admin') ? '8' : '7' }}" class="px-6 py-12 text-center">
                            <div class="flex flex-col items-center justify-center">
                                <svg class="w-12 h-12 mb-3 text-gray-300" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                                </svg>
                                <p class="font-medium text-gray-500">No passages found matching your search.</p>
                            </div>
                        </td>
                    </tr>
                @endforelse
            </tbody>
        </table>
    </div>

    @if ($passages->hasPages())
        <div class="px-4 py-3 bg-white border-t border-gray-200 pagination-wrapper">
            {{-- FIX: Append Params to Pagination --}}
            {{ $passages->appends($routeParams)->links() }}
        </div>
    @endif
</div>
