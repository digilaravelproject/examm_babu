...
@php
    ...
    // Pre-generate URLs
    $urlCreate = route($routePrefix . 'comprehensions.create', $routeParams);
...
@section('content')
    ...
    <div x-data="comprehensionManagement({
        baseUrl: '{{ $urlIndex }}'
        ...
        <div x-show="!loading" id="comprehensions-table-container">
            {{-- Pass variables to partial so it renders edit/delete links correctly --}}
            @include('admin.comprehensions.partials.table', ['routePrefix' => $routePrefix, 'routeParams' => $routeParams])
        </div>
...
@push('scripts')
    <script>
        ...
        function comprehensionManagement(config) {
            return {
                search: '',
                status: '',
                loading: false,
                currentPage: 1,
                baseUrl: config.baseUrl, // Use dynamic URL from PHP
                ...
                fetchData(page = 1) {
                    ...
                    const fetchUrl = `${this.baseUrl}?${params.toString()}`;
