...
@php
    ...
    // Prepare Parameters (needed for 'panel.' routes)
    $routeParams = [];
    if (!$isAdmin) {
        $routeParams = ['role' => request()->route('role') ?? 'instructor'];
    }

    // --- 2. Pre-generate URLs for JS/View ---
    $urlSample  = route($routePrefix . 'questions.import.sample', $routeParams);
    $urlPrepare = route($routePrefix . 'questions.import.prepare', $routeParams);
    $urlChunk   = route($routePrefix . 'questions.import.chunk', $routeParams);
    $urlIndex   = route($routePrefix . 'questions.index', $routeParams);
@endphp
                ...
                <form id="importForm" enctype="multipart/form-data" class="space-y-8">
                    ...
                    <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                        ...
                        <div class="relative p-6 transition-all duration-300 border border-gray-200 rounded-2xl bg-gray-50 hover:bg-white hover:shadow-md group">
                            <div class="flex flex-col h-full">
                                ...
                                <div class="mt-auto">
                                    {{-- FIX: Use Dynamic Sample URL --}}
                                    <a href="{{ $urlSample }}" class="flex items-center justify-center w-full gap-2 px-4 py-3 text-sm font-bold text-gray-700 transition-all bg-white border border-gray-300 shadow-sm rounded-xl hover:bg-gray-50 hover:text-[#0777be]">
                        ...
                        <div class="relative p-6 transition-all duration-300 border-2 border-blue-200 border-dashed rounded-2xl bg-blue-50/50 hover:bg-blue-50 group">
                            <div class="flex flex-col h-full space-y-4">
                                <div class="flex items-center gap-3 mb-2">
                                    ...
                                    <h3 class="font-bold text-gray-800">Select Topic & Upload</h3>
                                </div>

                                {{-- ADDED: Topic Dropdown --}}
                                <div class="bg-white p-3 rounded-xl border border-blue-100 shadow-sm">
                                    <label class="block text-xs font-bold text-gray-500 uppercase mb-1">Target Topic <span class="text-red-500">*</span></label>
                                ...
                                {{-- Custom Trigger --}}
                                <label for="fileInput" class="flex flex-col items-center justify-center flex-1 w-full h-24 mt-2 transition-all border-2 border-white border-dashed cursor-pointer rounded-xl hover:border-blue-400 bg-white/50 hover:bg-white" id="dropZone">
                                    <div class="text-center" id="emptyState">
                                        <svg class="w-8 h-8 mx-auto mb-1 text-blue-300 transition-colors group-hover:text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12"></path></svg>
                                        <p class="text-sm font-medium text-gray-600">Click to Browse</p>
                                    </div>

                                    {{-- Selected State --}}
                                    <div id="selectedState" class="hidden w-full px-4 text-center">
                                        <div class="flex items-center justify-center w-10 h-10 mx-auto mb-1 bg-green-100 rounded-full">
                                            <svg class="w-5 h-5 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                                        </div>
                                        <p id="fileName" class="text-sm font-bold text-gray-800 truncate">filename.xlsx</p>
                                        <p class="text-xs text-blue-500">Ready</p>
                                    </div>
                                </label>
                            </div>
                        </div>
                    ...
                    <div class="pt-4">
                        <button type="submit" id="submitBtn" disabled class="flex items-center justify-center w-full gap-2 py-4 text-lg font-bold text-white transition-all bg-gray-300 shadow-none cursor-not-allowed rounded-xl group">
                            <span>Select Topic & File First</span>
                        </button>
                    </div>
                </form>
            </div>
...
const topicSelect = document.getElementById('topicSelect'); // Added Topic Select

// Helper to check validation
function checkValidation() {
    if (fileInput.files && fileInput.files.length > 0 && topicSelect.value !== "") {
        ...
        submitBtn.classList.add('bg-gray-300', 'cursor-not-allowed', 'shadow-none');
        submitBtn.classList.remove('bg-[#0777be]', 'hover:bg-[#0666a3]', 'shadow-lg', 'hover:scale-[1.01]', 'cursor-pointer');

        if (topicSelect.value === "") {
            submitBtn.innerHTML = `<span>Select a Topic First</span>`;
        } else {
            submitBtn.innerHTML = `<span>Select a File</span>`;
        }
    }
}

// Event Listeners
topicSelect.addEventListener('change', checkValidation);
...
document.getElementById('importForm').addEventListener('submit', async function(e) {
    ...
    try {
        // STEP 1: Upload (Using Dynamic URL)
        let uploadRes = await fetch("{{ $urlPrepare }}", {
            method: 'POST',
            body: formData,
        ...
        // STEP 2: Process Loop (Using Dynamic URL)
        while (processed < total) {
            let chunkRes = await fetch("{{ $urlChunk }}", {
                method: 'POST',
                headers: {
        ...
        if (allErrors.length > 0) {
            ...
            // Redirect after delay (Using Dynamic Index URL)
            setTimeout(() => { window.location.href = "{{ $urlIndex }}"; }, 2000);
        }
