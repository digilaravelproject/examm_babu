@php
    ...
    // Merge Route Params with ID for Update
    $updateParams = array_merge($routeParams ?? [], ['category' => $category->id]);

    $action = $isEdit
        ? route($routePrefix . 'categories.update', $updateParams)
        : route($routePrefix . 'categories.store', $routeParams ?? []);
@endphp
...
<div class="max-w-5xl mx-auto overflow-hidden font-sans bg-white border border-gray-200 shadow-lg rounded-xl">
    <form action="{{ $action }}" method="POST" enctype="multipart/form-data" x-data="imagePreview()">
        ...
        @if ($isEdit)
            @method('PUT')
        @endif
        {{-- ... Rest of the form is identical to what you had ... --}}
        ...
        <div class="flex items-center justify-end gap-3 px-6 py-4 border-t border-gray-200 bg-gray-50">
            <a href="{{ route($routePrefix . 'categories.index', $routeParams ?? []) }}"
