        ...
        <div class="p-6 space-y-6">
            <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                ...
                <div class="space-y-1.5 {{ $isEdit ? '' : 'md:col-span-2' }}">
                    <label class="block text-xs font-bold tracking-wide text-gray-600 uppercase">Category Name <span class="text-red-500">*</span></label>
                ...
                <div class="space-y-1.5">
                    <label class="block text-xs font-bold tracking-wide text-gray-400 uppercase">Category Code (Read Only)</label>
            ...
            <div class="grid items-start grid-cols-1 gap-8 md:grid-cols-2">
                ...
                <div class="space-y-1.5">
                    ...
                    <label class="flex items-center justify-between p-3 bg-white border border-gray-200 rounded-lg shadow-sm cursor-pointer">
                        ...
                        <div class="relative">
                            <input type="checkbox" name="is_active" value="1" {{ old('is_active', $category->is_active ?? 1) ? 'checked' : '' }} class="sr-only peer">
            ...
            <div class="space-y-1.5">
                ...
                <textarea name="short_description" rows="2" class="w-full border-gray-300 rounded-lg p-2.5 text-sm focus:border-[#0777be] focus:ring-1 focus:ring-[#0777be] transition shadow-sm">{{ old('short_description', $category->short_description) }}</textarea>
            </div>
        </div>

        <div class="flex items-center justify-end gap-3 px-6 py-4 border-t border-gray-200 bg-gray-50">
            <a href="{{ route('admin.categories.index') }}" class="px-4 py-2 text-xs font-bold tracking-wide text-gray-500 uppercase">Cancel</a>
            <button type="submit" class="px-8 py-2.5 bg-[#0777be] text-white rounded-lg shadow hover:bg-[#0666a3] font-bold text-xs uppercase tracking-wide transition-all">
...
<script>
    function imagePreview() {
        return {
            imageUrl: '{{ $category->image_path ? asset('storage/'.$category->image_path) : '' }}',
            fileChosen(event) {
