...
@php
    ...
    $urlIndex = route($routePrefix . 'categories.index', $params);
    $urlCreate = route($routePrefix . 'categories.create', $params);
@endphp
...
@section('content')
    ...
    <div x-data="categoryManagement('{{ $urlIndex }}')" x-init="init()" class="space-y-6">

        {{-- Header Section --}}
        <div class="flex flex-col justify-between gap-4 md:flex-row md:items-center">
            <div>
                <h1 class="text-2xl font-bold tracking-tight text-gray-900">Categories</h1>
                <p class="mt-1 text-sm text-gray-500">Manage main categories, sub-categories, and their visibility.</p>
            </div>
        ...
        {{-- Table Container --}}
        <div x-show="!loading" id="categories-table-container">
...
@push('scripts')
    ...
    <script>
        ...
        function categoryManagement(urlIndex) {
            return {
                ...
                init() {
                    document.getElementById('categories-table-container').addEventListener('click', (e) => {
        ...
        function confirmDelete(id, name, subCount) {

            // 1. BLOCK DELETE if sub-categories exist
            if (subCount > 0) {
                Swal.fire({
                    title: '🚫 Action Blocked',
                    html: `
                        <div class="text-left text-sm text-gray-600">
                            <p class="mb-2">You cannot delete <b>"${name}"</b> because it has <b>${subCount}</b> linked Sub-Categories.</p>
                            <p class="text-red-500 font-bold">Please delete the sub-categories (and their micro-categories) first.</p>
                        </div>
                    `,
                    icon: 'error',
