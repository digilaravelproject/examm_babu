                    ...
                    <tr class="transition-colors hover:bg-gray-50/80 group">
                        ...
                        <td class="px-4 py-4">
                            ...
                            @if($skill->short_description)
                                <div class="text-xs text-gray-500 truncate max-w-xs">{{ $skill->short_description }}</div>
                            @endif
                        </td>
                        ...
                        <td class="px-4 py-4">
                            <div class="flex flex-col">
                                <span class="text-sm font-medium text-gray-700">
                                    {{ $skill->creator->fullname ?? 'System' }}
                                </span>
                                <span class="text-[10px] text-gray-400">
                                    {{ $skill->creator ? $skill->creator->getRoleNames()->first() : 'Admin' }}
                                </span>
                            </div>
                        </td>
                        ...
                        <td class="px-4 py-4 text-right">
                            <div class="flex items-center justify-end gap-2">
                                @php
                                    $canEdit = auth()->user()->hasRole('admin') || $skill->created_by == auth()->id();

                                    $editUrl = route($routePrefix . 'skills.edit', array_merge($routeParams ?? [], ['skill' => $skill->id]));
                                    $deleteUrl = route($routePrefix . 'skills.destroy', array_merge($routeParams ?? [], ['skill' => $skill->id]));
                                @endphp
                                ...
                                @if($canEdit)
                                    ...
                                    <button type="button" onclick="confirmDelete({{ $skill->id }}, '{{ addslashes($skill->name) }}')"
                                    ...
                                    <form id="delete-form-{{ $skill->id }}" action="{{ $deleteUrl }}" method="POST" style="display: none;">
                                        @csrf @method('DELETE')
                                    </form>
                                @else
