                    ...
                    <tr class="transition-colors hover:bg-gray-50/80 group">
                        ...
                        <td class="px-5 py-4 text-right">
                            <div class="flex items-center justify-end gap-2">
                                {{-- Only show if NOT Admin and NOT the current user --}}
@if(!$user->hasRole('admin') && auth()->id() !== $user->id)
    <form action="{{ route('admin.users.impersonate', $user->id) }}" method="POST" class="inline-block"
          onsubmit="return confirm('Login as {{ $user->name }}?');">
        @csrf
        <button type="submit"
                class="p-2 text-gray-400 transition-all rounded-lg hover:text-purple-600 hover:bg-purple-50"
                title="Login as User">
            {{-- Icon: User Switch / Eye --}}
