    ...
    <div x-data="userManagement()" x-init="fetchUsers()" class="space-y-6">
        ...
        <div class="flex flex-col justify-between gap-4 md:flex-row md:items-center">
            ...
            <div class="flex gap-3">
                {{-- Import Button --}}
                <button
                    class="flex items-center gap-2 px-4 py-2 text-sm font-semibold text-gray-700 transition-all bg-white border border-gray-300 rounded-lg shadow-sm hover:border-[#0777be] hover:text-[#0777be] hover:shadow-md">
            ...
            fetch(`/admin/users/${id}/toggle-status`, {
                ...
                .then(d => {
                    if (d.success) {
                        ...
                        Toast.fire({
                            icon: 'success',
                            title: `${name} is now ${d.new_status ? 'Active' : 'Inactive'}`
                        });
                    } else {
                        el.checked = !el.checked;
                        Toast.fire({
                            icon: 'error',
                            title: d.message || 'Error updating status'
                        });
                    }
                })
                .catch(() => {
                    ...
                    fetch(`/admin/users/${id}`, {
                        ...
                        .then(d => {
                            if (d.success) {
                                ...
                                // 1 second ruko taaki user message padh le, phir page REFRESH kro
                                setTimeout(() => {
                                ...
                                Toast.fire({
                                    icon: 'error',
                                    title: d.message || 'Error deleting user'
                                });
                            }
                        })
                        .catch(error => {
                            ...
                            Toast.fire({
                                icon: 'error',
                                title: error || 'Server Error (Check Console)'
                            });
                        });
                }
            });
