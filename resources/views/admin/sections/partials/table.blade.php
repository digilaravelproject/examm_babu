        ...
        <table class="w-full text-left border-collapse">
            <thead class="border-b border-gray-200 bg-gray-50">
                <tr>
                    ...
                    <th class="px-4 py-3 text-xs font-bold tracking-wider text-gray-500 uppercase">Section Name</th>
                    ...
                    <tr class="transition-colors hover:bg-gray-50/80 group">
                        ...
                        <td class="px-4 py-4">
                            ...
                            @if($section->short_description)
                                <div class="text-xs text-gray-500 truncate max-w-xs">{{ $section->short_description }}</div>
                            @endif
                        </td>
                        ...
                        <td class="px-4 py-4">
                            <div class="flex flex-col">
                                <span class="text-sm font-medium text-gray-700">
                                    {{ $section->creator->fullname ?? 'System' }}
                                </span>
                                <span class="text-[10px] text-gray-400">
                                    {{ $section->creator ? $section->creator->getRoleNames()->first() : 'Admin' }}
                                </span>
                            </div>
                        </td>
                        ...
                        <td class="px-4 py-4 text-right">
                            <div class="flex items-center justify-end gap-2">
                                @php
                                    $canEdit = auth()->user()->hasRole('admin') || $section->created_by == auth()->id();

                                    $editUrl = route($routePrefix . 'sections.edit', array_merge($routeParams ?? [], ['section' => $section->id]));
                                    $deleteUrl = route($routePrefix . 'sections.destroy', array_merge($routeParams ?? [], ['section' => $section->id]));
                                @endphp
                                ...
                                @if($canEdit)
                                    ...
                                    <button type="button" onclick="confirmDelete({{ $section->id }}, '{{ addslashes($section->name) }}')"
                                    ...
                                    <form id="delete-form-{{ $section->id }}" action="{{ $deleteUrl }}" method="POST" style="display: none;">
                                        @csrf @method('DELETE')
                                    </form>
                                @else
                                    <button disabled class="flex items-center justify-center w-8 h-8 border border-gray-200 rounded-lg bg-gray-50 btn-disabled" title="You cannot edit this section">
                    ...
                    <tr>
                        <td colspan="5" class="px-6 py-12 text-center">
                            <div class="flex flex-col items-center justify-center">
                                ...
                                <p class="font-medium text-gray-500">No sections found.</p>
                            </div>
                        </td>
                    </tr>
                @endforelse
