...
@php
    ...
    $urlIndex = route($routePrefix . 'sections.index', $params);
    $urlCreate = route($routePrefix . 'sections.create', $params);
@endphp
...
@section('content')
    ...
    <div x-data="sectionManagement('{{ $urlIndex }}')" x-init="init()" class="space-y-6">

        {{-- Header --}}
        <div class="flex flex-col justify-between gap-4 md:flex-row md:items-center">
            <div>
                <h1 class="text-2xl font-bold tracking-tight text-gray-900">Sections</h1>
                <p class="mt-1 text-sm text-gray-500">Manage exam sections (e.g., Logical Reasoning, Quant).</p>
            </div>
            <a href="{{ $urlCreate }}"
...
@push('scripts')
    ...
    <script>
        function sectionManagement(baseUrl) {
            return {
                search: '',
                status: '',
                loading: false,
