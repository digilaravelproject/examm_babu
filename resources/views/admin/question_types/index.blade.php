...
@section('header', 'Question Types')

@section('content')
    <style>
        /* Pagination buttons fix - Matching Reference Style */
    ...
    <div x-data="questionTypeManagement()" x-init="init()" class="space-y-6">

        {{-- Header --}}
        <div class="flex flex-col justify-between gap-4 md:flex-row md:items-center">
            <div>
                <h1 class="text-2xl font-bold tracking-tight text-gray-900">Question Types</h1>
                <p class="mt-1 text-sm text-gray-500">Manage and organize question formats like MSA, MMA, FIB, etc.</p>
            </div>

            {{-- Action Button (Optional - Uncomment if needed) --}}
            {{--
            <div class="flex gap-3">
                <a href="{{ route('admin.question_types.create') }}"
        ...
        {{-- Filters (Sync UI with Reference) --}}
        ...
        {{-- Table Container --}}
        <div x-show="!loading" id="types-table-container">
            @include('admin.question_types.partials.table')
        </div>
...
@push('scripts')
    <script>
        ...
        function questionTypeManagement() {
            return {
                search: '',
                status: '',
                loading: false,
                ...
                fetchData(page = 1) {
                    ...
                    const url = "{{ route('admin.question-types.index') }}";
                    ...
                    fetch(fetchUrl, {
                        ...
                        .then(r => r.text())
                        .then(html => {
                            document.getElementById('types-table-container').innerHTML = html;
                            this.loading = false;
                        })
                        .catch(err => {
                            console.error(err);
