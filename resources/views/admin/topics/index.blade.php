...
@php
    ...
    $urlIndex = route($routePrefix . 'topics.index', $params);
    $urlCreate = route($routePrefix . 'topics.create', $params);
@endphp
...
@section('content')
    ...
    <div x-data="topicManagement('{{ $urlIndex }}')" x-init="init()" class="space-y-6">
        ...
        <div class="p-1.5 bg-white border border-gray-200 shadow-sm rounded-xl flex flex-col md:flex-row gap-3">
            ...
            <select x-model="status" @change="applyFilter()"
                class="py-2.5 text-sm bg-gray-50 border-0 rounded-lg md:w-40 cursor-pointer hover:bg-gray-100">
        ...
        <div x-show="!loading" id="table-container">
            @include('admin.topics.partials.table', [
...
@push('scripts')
    ...
    <script>
        function topicManagement(baseUrl) {
            return {
                search: '',
                status: '',
                skill_id: '',
                ...
                fetchData(page = 1) {
                    ...
                    if(this.skill_id) params.append('skill_id', this.skill_id);
