...
class UserExamRepository
{
    ...
    public function evaluateAnswer(Question $question, $userAnswer)
    {
        ...
        // 2. Match The Following Logic (Option Matching)
        if ($typeCode === 'MTF') {
            if (!is_array($userAnswer)) return false;

            $originalOptions = $question->options;
            // Handle if options are array or object
            if (is_string($originalOptions)) {
                $originalOptions = json_decode($originalOptions, true);
            }

            foreach ($userAnswer as $index => $userSelectedRightId) {
                // Ensure IDs match the original position's ID
                $userVal = (int) $userSelectedRightId;

                // Fallback: If DB doesn't have ID in JSON options, use index
                $expectedId = isset($originalOptions[$index]['id']) ? $originalOptions[$index]['id'] : $index;

                if ($userVal != $expectedId) {
                    return false;
                }
            }
            return true;
        ...
        if ($typeCode === 'ORD') {
            ...
            // The Correct Answer is the sequence of IDs in the order they appear in DB options.
            foreach ($originalOptions as $index => $opt) {
                // The user's item at $index MUST match the original item's ID at $index
                $expectedId = isset($opt['id']) ? $opt['id'] : $index;
                $userVal = isset($userAnswer[$index]) ? (int)$userAnswer[$index] : -1;
        ...
        // 4. Short Answer Logic (SAQ)
        if ($typeCode === 'SAQ') {
            $originalOptions = $question->options;
            if (is_string($originalOptions)) $originalOptions = json_decode($originalOptions, true);

            $userText = trim(strtolower((string)$userAnswer));

            foreach($originalOptions as $opt) {
                $possibleAnswer = trim(strtolower($opt['option'] ?? ''));
                if ($possibleAnswer === $userText) {
                    return true;
                }
            }
            return false;
        ...
        // 5. Default Handlers (MSA, MMA, TOF)
        ...
        switch ($typeCode) {
            case 'MMA':
                ...
                $u = array_map('intval', $userAnswer);
                $c = array_map('intval', $correctArr);
                sort($u); sort($c);
                return $u == $c;
